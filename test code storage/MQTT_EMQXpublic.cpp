/**
 * Adapted from Milestone 1.cpp (MQTT_AWS.cpp) and credentials.h
 */
...
void loop() {
    ...
    Serial.println(client.state());
    if (!client.loop()){                                                         // https://pubsubclient.knolleary.net/api#loop. If the client is not connected, attempts to connect again.
        Serial.println(client.state());
        connectEMQX();
    }                                                                             
